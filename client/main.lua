    ...
    for i=1, #Config.Locations do
        ...
        if blip then
            if not DoesBlipExist(blip["id"]) then
                ...
                SetBlipSprite(blip["id"], blip["bid"])
                SetBlipDisplay(blip["id"], 4)
                SetBlipScale(blip["id"], blip["bscala"])
                SetBlipColour(blip["id"], blip["bco"])
                SetBlipAsShortRange(blip["id"], true)

                BeginTextCommandSetBlipName("shopblip")
                AddTextEntry("shopblip", blip["nombre"])
